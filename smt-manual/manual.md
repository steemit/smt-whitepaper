...
## SMTs Benefit from Blockchains with Scaling Processes that are Programmed to a Specialized Set of Applications

Blockchain scaling is an integral part of discussions that uncover the viability of a given blockchain to serve market demand from competeing applications and use cases.  As an industry, we have some clues that may guide us to the most optimal scaling strategies, such as, after finding product-market fit, it is a general principle that specialized platforms scale better than generalized platforms or platforms that continue to try to serve applications outside of their wheelhouse.  In blockchain, there are new concepts of "sharding" and "mutli-threaded paralleism" that refer to how blockchains may scale by allowing multiple processes to occur at once.  General purpose platforms, such as Ethereum, are a great test bed for these apporaches to scaling, however, a platform that takes advanatage of all the product-market fit discovered by Ethereum that then applies it to a more specialized, hard-coded, iterative-upgrading model, such as Steem, can scale its processes more effectively to meet the demand discovered by that product-market fit.  

Looking to the 90s and early 00’s for analogy, when the computer science world started writing code specifically optimized for GPUs, the boundary pushing for greater scale occurred through FPGAs - field programmable gate arrays, which are essentially processors for programming how all the gates of information operate with each other. This is not quite ASIC but significantly faster than GPU or CPU. As these platforms move to more and more generalizations, such as the idea that any contract may call on any other contract, they will move further away from ability to optimize for scale, as contracts that call on all other contracts will reduce the capacity for multi-paralell processing to single-core processing. By analogy, like CPUs can never optimize better than GPUs, platforms like Ethereum, GEOS and Tezos can never optimize better than a hard-coded, specialized blockchain like Steem.  These CPU-like blockchains will be bottlenecked by unpredicatable processing requirements, while the ultimate blockchain platforms will be specially-designed, like Steem, and will scale by optimizing in the way FPGAs were optimized in GPUs.
