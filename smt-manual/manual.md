...
Looking to the 90s and early 00’s for analogy, when the computer science world started writing code specifically optimized for GPUs, the boundary pushing for greater scale occurred through FPGAs - field programmable gate arrays, which are essentially processors for programming how all the gates of information operate with each other. This is not quite ASIC but significantly faster than GPU or CPU.  By analogy, platforms like Ethereum, GEOS and Tezos, which are general, like CPUs, can never optimize in this way for scaling.  These CPU-like blockchains will be bottlenecked while the ultimate blockchain platforms will be specially-designed, like Steem, and will scale by optimizing in the way FPGAs were optimized in GPUs. Steem can continue to optimize and achieve high level of parallelism that cannot be destroyed by attack, such as contract to contract calling across threads, or user intervention.
