...
In blockchain scaling there are cutting-edge concepts of "sharding" and "multi-threaded parallelism" that refer to how blockchains may scale by allowing multiple processes to occur at once.  General purpose platforms, such as Ethereum, are a great test bed for these approaches to scaling, however, a platform that takes advantage of all the product-market fit discovered by Ethereum that then applies it to a more specialized, hard-coded, iterative-upgrading model, such as Steem, can scale its processes more effectively to meet the demand discovered by that product-market fit. 
